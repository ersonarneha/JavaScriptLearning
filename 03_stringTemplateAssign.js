var stringHandsOn = function () {
  ...
  console.log(
    `total number of char in string before removing spaces:${myString.length}`
  );

  var newString = `${myString.trim()}`;
  console.log(
    `Total no of characters after removing spaces:${newString.length}`
  );
  console.log(`No of extra spaces removed:10`);

  console.log(
    `first character after trim:${newString.charAt(
      0
    )},last character after trim:${newString.charAt(33)} `
  );
